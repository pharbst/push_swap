...
/*   Updated: 2022/08/23 16:29:21 by pharbst          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../push_swap.h"

static void	ft_helper_help1(t_stack **node, t_var **vars)
{
...
static void	ft_helper_help2(t_stack **node, t_var **vars)
{
	if ((*node)->index < (*vars)->midpoint)
	{
		(*vars)->x = 1;
...
static void	ft_locate_chunk_helper(t_stack *stack, t_stack **node,
		t_var **vars)
{
	int	stacklen;
	...
	while ((*node)->chunk == (*vars)->chunkid && --stacklen >= 0)
		ft_helper_help1(node, vars);
	(*vars)->x = 0;
	while ((*node)->chunk != (*vars)->chunkid && stacklen != 0)
		(*node) = (*node)->next;
	while ((*node) != stack && stacklen != 0)
		ft_helper_help2(node, vars);
}
...
void	ft_locate_chunk(t_stack *stack, t_var **vars)
{
	...
	ft_locate_chunk_helper(stack, &stack, vars);
